...
- **BUILTIN 離線種子**：由 `tools/refresh-builtin-snapshot.js` 產生的 Tier 3 `shell/v136/builtin-snapshot.js`，包含完整八 key 並以 `BUILTIN_ASSET_VERSION` 綁定 App／SW；`shell/v136/index.html` 只保留 marker 與安全 boot guard，不複製 payload。root `index.html`／`app-version.js` 是 byte-locked v110 bridge，只在 v114 安裝前／失敗時續命。七張非 Ledger 資料來自 `schema.js` 指定的公開 CSV，Ledger 永遠只有 schema 推導的 21 欄空 header，禁止抓取 live Ledger 紀錄。asset 缺失／錯版時只接受有效 local snapshot，否則顯示 recovery 而非空 DB。預設 preview 只讀，經 Bar 核准才可 `--write`；完整 SOP 見 `16_OPS_PLAYBOOK.md` §G，決策見 ADR 0019。
