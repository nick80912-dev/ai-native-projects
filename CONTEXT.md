...
- **BUILTIN 離線種子**：由 `tools/refresh-builtin-snapshot.js` 產生的 Tier 3 `shell/v123/builtin-snapshot.js`，包含完整八 key 並以 `BUILTIN_ASSET_VERSION` 綁定 App／SW；`shell/v123/index.html` 只保留 marker 與安全 boot guard，不複製 payload。root `index.html`／`app-version.js` 是 byte-locked v110 bridge，只在 v114 安裝前／失敗時續命。七張非 Ledger 資料來自 `schema.js` 指定的公開 CSV，Ledger 永遠只有 schema 推導的 21 欄空 header，禁止抓取 live Ledger 紀錄。asset 缺失／錯版時只接受有效 local snapshot，否則顯示 recovery 而非空 DB。預設 preview 只讀，經 Bar 核准才可 `--write`；完整 SOP 見 `16_OPS_PLAYBOOK.md` §G，決策見 ADR 0019。
...
- **採買明細**：區段標題為「代購對象與記帳紀錄」。對象姓名與數量同行，待買顯示 `需求 3 包 · 待買 1 包`，已買顯示 `需求 3 包 · 已買 2 包`；窄螢幕可自然換行。採買清單仍是個人、本機資料，記帳進度只計 allocation 對應的 Ledger 品項，單位一律用「筆」，不得用「位／對象」暗示付款人或團體成員。原獨立「記帳進度」列合併進「狀態」列，例如 `已買 · 未記帳`、`已買 · 已記帳 1／3 筆`；混合待確認時逐類顯示筆數。未記帳且無待確認時按鈕為「記帳」或「繼續記帳（剩 N 筆）」；只要有 allocation 待確認，就顯示原生 disabled 的「等待狀態確認」與原因，避免先提供實際會被 workflow preflight 阻擋的入口。下方逐 allocation 狀態、已記帳明細導航與「改回未記帳」仍完整保留。
...
- **Shopping UI workflow state seam**：`shopping-ui-state.js` 是待買／已買 tab、selection mode、selected truth map，以及採買表單 `form`／`formSession`／`photoError` lifecycle 的唯一 transition 權威，公開 `createState(seed)`、`transition(state, action)` 與 `createWorkflow(adapter)`。Production adapter 必須先 commit state 再依序執行 split cleanup、mount/render、pending、返回與 focus effects；以 runtime 產生的 session/request ID 阻擋 stale save／photo completion。Shopping page filters、detail state、split payload、Shopping store、photo repository、Buy-to-Ledger domain、資料格式與 renderer markup 均不屬於此 seam；批次失敗、preflight 阻擋或取消時不得 reset selection，form validation/store/photo failure 也不得關閉 Sheet。
