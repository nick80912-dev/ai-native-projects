...
- **Shopping list tab／selection workflow state seam**：`shopping-ui-state.js` 是待買／已買 tab、selection mode 與 selected truth map 的唯一 transition 權威，公開 `createState(seed)`、`transition(state, action)` 與 `createWorkflow(adapter)`。Production adapter 只從既有 `shoppingUiState` 投影／回寫這三欄，且必須先 commit state 再依序執行 split cleanup、list render 與 focus。Shopping page filters、form／formSession、detail、split payload、photoError、Shopping store、photo repository、Buy-to-Ledger domain、資料格式與 renderer markup 均不屬於此 seam；批次失敗、preflight 阻擋或取消時不得 reset selection。
