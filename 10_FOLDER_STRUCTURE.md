...
> root `index.html`／`app-version.js` 是 byte-locked v110 bridge；current App 位於 `shell/v136/`，只有 v114 worker 成功啟用後才接管 root 導覽。裝置照片、採買轉記帳、Ledger UI、Shopping UI 與 Trip progression 各有獨立 runtime 邊界。
...
shell/v136/index.html   current App:UI 殼 + CSS + 內嵌 JS（依區塊分層，見下）
shell/v136/app-version.js current App 顯示版本（必須與 sw.js 的 SW_VERSION 一致）
shell/v136/builtin-snapshot.js 工具產生、與 current App／SW 同版的 BUILTIN 離線資產（Tier 3，禁手改）
...
```
> App UI、DOM adapter 與多數流程維持在單一 current generation document `shell/v136/index.html`；獨立 runtime modules 由 `runtime-assets.json` 登錄並以 `tools/check-runtime-assets.js` 驗證入口、SW 與文件一致性。root bridge 不是日常功能修改目標。

## Current App HTML `shell/v136/index.html` 內部分層(區塊順序,即邏輯模組)
```
SCHEMA        來自 schema.js(pubBase + sheets.*.gid + 欄位/型別規格)
BUILTIN       由 shell/v136/builtin-snapshot.js 載入的 8 表離線後備；current HTML 只留安全 guard
...
shell/v136/                current App document／version／generated BUILTIN generation
