...
> 常見任務的標準步驟鏈。任務級別(A 純資料 / B 顯示層 / C 架構資料部署)與對應流程見 `15_AI_EXECUTION_RULES.md` §5;C 級走 PROJECT_CONSTITUTION 十三步。涉及 Tier 2 高風險檔案時,疊加 `14_FILE_TIERS_AND_GATE.md` 的確認程序。
...
## 情境 F:建立本機開發環境清理排程
> 詳細安全規範已移至 `16_OPS_PLAYBOOK.md` §B。重點:先建立**服務登記檔**(未登記一律不清)、dry-run 為第一步、白名單策略、log/自測/移除方式齊備、每 6 小時一次;禁以廣泛 process name 終止行程。
