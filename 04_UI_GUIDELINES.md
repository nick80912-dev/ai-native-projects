...
- **正圓圖示鈕吃 `--coral`(accent),不吃 `--action-cta-bg`(Bar 2026-09-23 裁定)**。`.ledger-fab`、`.shopping-add-button` 這類純圖示圓鈕屬**accent 裝飾層**,不屬動作角色 —— 依本檔「形狀語意」節,正圓自成一類、本來就不套 `.btn`,吃不同的色是一致的。
  - **因此在海洋與象牙兩組主題下,＋ 圓鈕與「新增第一項採買」會差一階深淺**(海洋 `#df5f3a` vs `#c05232`、象牙 `#e25a0f` vs `#c74f0d`)。**這是預期行為,不是漏網。** 其餘四組的 `--t-cta-bg` 等於 accent,兩者同色。
  - 成因:v125 只把白字未過 AA 的那兩組 accent 壓深作為 CTA 底色;圓鈕上沒有白色文字標籤(只有一個 `＋` 字符),不受該對比要求約束。
  - **不要為了「一致」把圓鈕改吃 `--action-cta-bg`** —— 那會連帶改動 `.ledger-fab`,範圍大於表面所見,且會讓「正圓＝裝飾層」這條界線失效。
...
- **Sheet 儲存格裡的換行是作者的排版意圖,顯示時以 `white-space:pre-line` 保留**,不要讓它折成空白。適用元件目前為 `.detail .sec`(資訊面板)、`.drive-chip`(交通 chip)與 `.sm-hours-list`(商場營業時間,v134)。新增會渲染 CMS 自由文字的元件時比照辦理。
- **商場營業時間的 ` / ` 是分段符號,逐段分行顯示**(v134)。資料中 57 筆營業時間只有 P001 永旺、P039 Ario 兩筆使用 ` / `,其餘沒有任何 `/`,故以 ` / `(前後含空格)切分不會誤傷。餐廳常見的 `11:00-14:00、17:00-21:00` 是同一項目的兩個時段,**不得**依 `、` 切分。
