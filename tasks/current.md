...
| **dev 候選版** | **SW v84**,`app-version.js` 與 `sw.js` 均為 v84 |
| 個人備份格式 | **v9**(`PERSONAL_STATE_SUPPORTED_VERSIONS = [1..9]`)—— v81 因想逛 key 識別語意變更而升版 |
| dev 自動驗證 | **65／65** Node test files、Playwright **92／92**、`check-doc-titles` 與 `check-app-version` 通過 |
| 既有 tag | `production-v18`、`production-v73` |

**`dev` 領先 `main` 十一個候選版(v74–v84),全部尚未正式發布。** v74 起的每一批都通過完整自動驗證,但只有 v73 經過 Bar 真機驗收與正式部署。
...
| # | Gate | 狀態 |
|---|---|---|
| G1' | v74–v84 累積 delta 的 Bar 真機／PWA 驗收 | ⬜ **未開始** |
| G4' | Bar 核准 PR merge `dev → main` | ⬜ 未開始 |
| G5' | 正式站部署後線上驗證 | ⬜ 未開始 |
| G6' | 建立 `production-v84`(或屆時版本)tag | ⬜ 未開始 |
...
| v85 | 資料可感知性:今日支出與旅程累計、同步相對時間與 partial 失敗來源、個人／團體脈絡分離、設定頁資料健康摘要 | ⬜ 未開始 |
| v86／v87 | SW 更新提示 —— **需兩個版本才能完成驗收**:v86 加入監聽與提示,v87 作為真實更新目標。不修改 SW 生命週期與快取策略,只做正常版本遞增 | ⬜ 未開始 |
...
→ **v85「資料可感知性」批的計畫與實作。** v82–v84 已完成本機實作與完整自動驗證,待 Bar 真機驗收。
