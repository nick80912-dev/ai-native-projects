...
## ⏸ 等 Bar 動作
1. 以手機全面驗收單幣換算、共享匯率、分帳同步、離線補送、TEST 排除、雙擊不縮放、快速類別／幣別／設定控制項點擊、Schema 2.5 與 SW v16。
2. 驗收通過後核准 PR merge `dev → main`。

## 下一棒
→ 完成手機實機驗收；若 no-op listener 有快速誤觸副作用，移除並 bump SW v17 回滾。通過後 merge，Netlify 正式部署與線上驗證(回滾見 16 §A)。
