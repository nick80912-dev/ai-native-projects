...
- [x] 共用今日三態：今天有消費只顯示「今天」日期摘要；今天零筆且有歷史紀錄顯示淡色「今日尚無消費」；完全無紀錄只顯示原空狀態。
- [x] 結算卡改為標題、主要狀態金額、同列進度／查看結算；應收／應付為「尚待 N 人」、我已結清為「團體尚待 N 人」、全員已結清為「目前沒有待處理款項」。首頁移除 `model.details`，Sheet 明細不變。
- [x] 個人代購卡與團體結算卡移除雙卡固定最小高度，使用 scoped `min-height:0`、10px／12px padding 與自然內容高度；整卡／文字按鈕觸控入口維持至少約 44px。
- [x] 未修改待同步面板、repository queue API、single-flight coordinator、Apps Script、日期分組、付款／分攤 Badge、代購計算或新增／編輯／刪除流程。
- [x] Service Worker cache 僅由 v41 順延至 v42；SHELL、install／activate／fetch 不變。首頁、結算、代購、同步、日期與既有行動版 SW 目標測試已通過。
- [x] 提交前完整 `tests/*.test.js` 僅執行一次：40／40 通過；文件標題、diff check 與禁止範圍檢查均通過。
- [ ] 等待 Bar iPhone Safari／PWA 真機驗收：375px／390px 卡片自然高度、長金額／TWD、底部同列、今日提示高度、日期雙幣摘要、整卡點擊、無水平捲動及 SW v42 更新。
...
- [x] 團體今日卡改為全寬，個人今日／代購雙卡不變；團體最近消費顯示付款者與分攤人數，付款者不再於 metadata 重複。
- [x] 「查看結算」仍沿用既有結算 Sheet，首頁沒有第二個結算入口；SW cache 僅由 v39 順延至 v40。
- [x] 40 個 `tests/*.test.js` 與文件標題檢查全部 exit 0；375px／390px Browser QA 確認團體結算／今日卡等於 Dashboard 全寬、唯一結算入口可開啟既有 Sheet，個人雙卡不變、`scrollWidth == clientWidth`、browser error 0。
