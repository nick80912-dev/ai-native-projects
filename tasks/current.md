...
| **dev 候選版** | **SW v89**,`app-version.js` 與 `sw.js` 均為 v89 |
| 個人備份格式 | **v9**(`PERSONAL_STATE_SUPPORTED_VERSIONS = [1..9]`)—— v81 因想逛 key 識別語意變更而升版 |
| dev 自動驗證 | **65／65** Node test files、Playwright **115／115**、`check-doc-titles` 與 `check-app-version` 通過 |
| 既有 tag | `production-v18`、`production-v73` |

**`dev` 領先 `main` 十六個候選版(v74–v89),全部尚未正式發布。** v74–v87 已由 Bar 於 2026-08-03 確認真機驗收皆正常；v88–v89 已通過完整自動驗證。只有 v73 已合併、正式部署並建立 production tag。
...
| G1'' | v88–v89 畫面／真機確認 | ⬜ dev 候選版待 Bar 後續檢視 |
| G4' | Bar 核准 PR merge `dev → main` | ⬜ 未開始 |
| G5' | 正式站部署後線上驗證 | ⬜ 未開始 |
| G6' | 建立 `production-v89`(或屆時版本)tag | ⬜ 未開始 |
...
| v90／v91 | SW 更新提示 —— **需兩個版本才能完成驗收**:v90 加入監聽與提示,v91 作為真實更新目標。不修改 SW 生命週期與快取策略,只做正常版本遞增 | ⬜ 未開始 |
...
→ **請 Bar 檢視 v89「代購標記同行」效果。** v74–v87 真機驗收已確認正常；v88–v89 已通過完整自動驗證並推至 `dev`。後續再決定是否推進 v90／v91 的 SW 更新提示雙版本驗收。
