...
> 更新於 2026-08-03。細任務層;里程碑看 `06_ROADMAP.md`,**逐版交付紀錄一律看 `07_CHANGELOG.md`**,正式待辦看 `tasks/backlog.md`。
...
| **dev 候選版** | **SW v88**,`app-version.js` 與 `sw.js` 均為 v88 |
| 個人備份格式 | **v9**(`PERSONAL_STATE_SUPPORTED_VERSIONS = [1..9]`)—— v81 因想逛 key 識別語意變更而升版 |
| dev 自動驗證 | **65／65** Node test files、Playwright **114／114**、`check-doc-titles` 與 `check-app-version` 通過 |
| 既有 tag | `production-v18`、`production-v73` |

**`dev` 領先 `main` 十五個候選版(v74–v88),全部尚未正式發布。** v74–v87 已由 Bar 於 2026-08-03 確認真機驗收皆正常；v88 已通過完整自動驗證，正等待畫面效果確認。只有 v73 已合併、正式部署並建立 production tag。
...
| G1'' | v88 資料可感知性的畫面／真機確認 | ⬜ 等待 Bar 檢視本次效果 |
| G4' | Bar 核准 PR merge `dev → main` | ⬜ 未開始 |
| G5' | 正式站部署後線上驗證 | ⬜ 未開始 |
| G6' | 建立 `production-v88`(或屆時版本)tag | ⬜ 未開始 |
...
| v88 | 資料可感知性:今日支出與旅程累計、精簡同步標籤與 partial 失敗來源、個人／團體脈絡分離、設定頁資料健康摘要 | ✅ 本機已交付，待 Bar 檢視效果 |
| v89／v90 | SW 更新提示 —— **需兩個版本才能完成驗收**:v89 加入監聽與提示,v90 作為真實更新目標。不修改 SW 生命週期與快取策略,只做正常版本遞增 | ⬜ 未開始 |
...
→ **請 Bar 檢視 v88「資料可感知性」效果。** v74–v87 真機驗收已確認正常；v88 通過完整自動驗證後維持本機待審。核准後再決定是否推進 v89／v90 的 SW 更新提示雙版本驗收。
