# BACKLOG(待辦,依優先序)

> 更新於 2026-09-12(v116 後)。做完或經 Bar 裁定不再需要的項目移到 done.md,正在做的移到 current.md。

> **編號刻意不連續,不得重排**:`tasks/current.md`、`tasks/done.md` 與 `07_CHANGELOG.md` 都以編號互相引用,重排會打斷既有交叉引用。已歸檔項目的編號一律**留空不回收**(目前缺號:1、2、3b、4、5、6–11、13–19、21–24、26、27、30–37、39);新項目接在現有最大號之後。

## 中優先(已核准正式待辦)
3. **驗收後 UI/內容微調**(最小修改,不動 schema)。
12. **決策記錄**:個人預算功能不做;多旅程平台化延後至旅程結束,併入框架抽取階段。
43. **同一天的進度數字,三個畫面互相矛盾 —— 已診斷,非資料錯誤,待裁定呈現口徑**(2026-09-23 於 v126 實測,時間模擬 Day 2／10-19 11:00):

    | 畫面 | 顯示 |
    |---|---|
    | 今天頁 hero | **1 / 8** |
    | 行程頁 day-head | 完成 **0**・略過 3 ／共 **14** |
    | 行程頁實際渲染 | **11** 張 `.item` |

    - **使用者看得到的矛盾**:今天頁說「1」,切到行程頁「完成」變成「0」。總數則有 8／14／11 三個版本。
    - **⚠ 2026-09-23 追查結論:三個數字各自都是對的,不存在計數錯誤。** 原本列為 BUG,現修正為**呈現口徑問題**。分母來自兩個不同的 filter,分子對「略過」的處理也不同:

      | | 分母來源 | 條件 | 值 |
      |---|---|---|---|
      | hero | `homeNextStopItems(day.items)` | 只取 `item.act`,**且把串點併成一站**(`clusterController`) | 8 |
      | day-head | `day.items.filter(isTripCheckableItem)` | `act || place || ref` 任一即算,**串點子站各算一站** | 14 |
      | 渲染 | `tripHideDone` 濾掉已清除項 | 14 − 3 略過 | 11 |

      分子差異更關鍵 —— hero 的 `completed = items.length - (pick.remaining + (pick.item?1:0))` 是「**已經走過的**」,**把自動略過也算進去**;day-head 的 `doneN` 只算真的打卡/完成(`!skip[id] && (checks[id] || progress.done[id])`),略過另以 `skipN` 顯示。所以 hero 的「1」就是那個被自動略過的站。**11 張卡片也完全解釋得通**(隱藏已完成把 3 個略過的濾掉)。

    - **最值得注意的一點:hero 那個數字已經有正確的無障礙標籤**,措辭是誠實的 ——

      ```js
      var progressLabel='今日已處理 '+completed+' 站，共 '+items.length+' 站';
      '<div class="loc" aria-label="'+escapeHtml(progressLabel)+'">'+completed+' / '+items.length+'</div>'
      ```

      **「已處理」**正是那個能和「完成」區分、涵蓋略過的詞。但畫面上只印 `1 / 8` —— **螢幕閱讀器使用者拿到的資訊比看得見的人更準確**。

    - **修法幾乎免費,且不需動任何計數邏輯**:把已存在的措辭搬到看得見的地方(例如 hero 顯示「已處理 1/8」),或在 day-head 補上串點口徑說明。**選哪種措辭是 Bar 的裁定,不是實作細節。**
    - **不要「統一」成同一個數字** —— 兩個分母各有用途(hero 服務下一站流程、day-head 服務打卡清單),硬統一會讓其中一邊失去意義。這也呼應 #31／#37 的教訓:看起來像疏漏的,先查清楚再動。
    - 屬 Tier 2,需 forward bump。來源:2026-09-23 新模型 active-trip 實機走查 + 同日原始碼追查。

## 低優先(未來,不急)
20. **SW SHELL 快取清單補齊評估**:重新盤點現行 App Shell 必要資產與離線回歸範圍，不沿用已作廢的 ZIP 打包流程。
25. **AI Native Framework 抽取**:App 穩定落地後執行,見 `FUTURE_PLAN_framework-extraction.md`。
28. **打卡控制觸控目標過小**:行程頁的 `.chk` 打卡方塊為 **24×24px**,整列不是熱區(往右 80px 落在 `.item-main`,不觸發)。`04_UI_GUIDELINES.md` 自訂的門檻是「所有清單觸控列 ≥44px 高」,24×24 約為建議面積的 30%,而這是走路中單手操作的元件。同頁 `.qa-btn` 為 38–41px,但準則對它有 ≥38px 的明文豁免,不在此項範圍。**Bar 於 2026-09-10 裁定暫不處理。** 屬 Tier 2,動工前需四項確認並 forward bump。
29. **多處字級低於準則下限**:`04_UI_GUIDELINES.md` 寫「輔助 11-13px」,實測行程頁 `.dow` 星期為 **9.5px**(6 處)、`.drive-chip`／`.tag` 10.5px(13 處)、今天頁 `.h-lbl` 10.5px(6 處)、購物頁 `.fl-arw` 10px(11 處)、分帳 `ledger-status-pill`／`ledger-summary-helper` 10px。改動散布廣,需逐一評估會不會撐破既有版面。**Bar 於 2026-09-10 裁定暫不處理。** 屬 Tier 2。
    - **2026-09-23 複審補量(v126)**:全 stylesheet 共 **45 處** `font-size < 11px`,最小 **8.5px**,分佈 8.5／9／9.5／10／10.5px。原記錄只涵蓋其中一部分。清除死 CSS 後這個數字已是活規則的實數。**本項範圍因此比原記述大**,動工前重新盤點。

38. **重新評估 `--entry-secondary-bg` 是否該主題化(Bar 2026-09-12 裁定:待實機確認後再決定)**:`#f3f8f6` 目前是**核定的非主題呈現 token**,`presentationTokens` 有斷言把關,`04_UI_GUIDELINES` 亦明載。backlog #31 就是因為誤判它為疏漏而被關閉。**本項不主張推翻,只記錄一個新事實供重新評估。**

    - **新事實:它的使用者在 2026-09-12 當天由 4 條增為 9 條**(v115 四條 → v116 五條 → v118 八條 → v119 九條)。當初核定「固定」時它只出現在少數幾處;**現在它是記帳 sheet 的主要背景語言**(摺疊標題、展開面板、預覽區、分攤成員、代購標記)。取捨的前提已經變了。
    - **實測色相關係**:`#f3f8f6` 為 H156／S26%／L96% 的冷調白。與各主題紙底的色相差:cedar **45°**、mist **46°**(可接受),但 ocean 114°、ivory 108°、tea 122°、wisteria 129°(對立色族)。**六組裡只有 2 組真的搭。**
    - **緩解因素**:S26%／L96% 幾乎是白的,不搭程度遠低於 `--mint` 當初的 S28%／L87.5%。對比不是問題。
    - **若決定主題化**:做法比照 v121 的 `--t-select-bg` —— 新增第一層 token、由各主題 `--t-action` 混白推導、以固定的對白距離確保各組表現一致,並納入 `theme-system.test.js` 的對比斷言。v121 已證實這條路可行。
    - **建議的判斷方式**:切到焙茶或象牙實際跑一輪記帳再決定。**色相差 120° 在 S26% 下未必難看,數字會誇大問題。**

    - **狀態(2026-09-12)**:**Bar 明示先保留,待實機看過再決定**。本項不是懸而未決的疑問,而是**刻意等待一個特定輸入** —— 焙茶／象牙下實際使用的觀感。在那之前不應因為色相數字好看或難看而自行推進。
    - **前車之鑑**:同日的 #31 與 #37 都是「看起來像疏漏就去修」而被自動測試擋下的前提錯誤。`--entry-secondary-bg` 與它們同屬**已核定的固定色**,推翻需要的是明確裁定,不是分析。

40. **鍵盤焦點樣式覆蓋率過低**:全 App **235 個 `<button>`,只有 13 條 `:focus-visible` 規則**,而且全是個別元素的窄選擇器(`.chk`／`.trip-back-now`／`.nx-buy-badge`／`.store-row`…),沒有 `.btn:focus-visible` 這種通則。

    - **現況不是「完全沒有焦點環」**:`outline:none` 只有四處,且都配了替代樣式;其餘按鈕仍吃瀏覽器預設焦點環。問題是**不一致** —— 13 顆有自訂的 2px `var(--sea)` 環,其餘看瀏覽器臉色。
    - **風險集中在深色實心鈕**:`.btn.cta`(各主題 CTA 底色)與 `.btn.danger`(固定紅 `#8c1d2c`)上,預設焦點環的對比通常很差。v125 才把這兩類的底色定深,這個問題隨之變明顯。
    - **成本低**:加一條 `.btn:focus-visible{outline:2px solid ...;outline-offset:2px}` 通則即可覆蓋主要缺口;深色底可考慮改用 `outline-color:#fff` 或雙層 `box-shadow`。屬 Tier 2,需 forward bump。
    - 來源:2026-09-23 新模型全專案複審。

41. **Ledger 表單一批觸控目標低於 44px**:`.ledger-participant-choice` **28px**、`.ledger-proxy-add-row button` 30px、`.ledger-category-apply-actions button` 34px、`.ledger-history-compact-options .ledger-sheet-choice` 34px、`.ledger-option-row button`／`.ledger-action-popover button`／`.ledger-form-open-choice`／`.ledger-entry-heading-row .ledger-sheet-toggle` 36px、`.travel-note-kind button` 36px、`.settings-identity-actions .btn` 38px。

    - **與 #28 的分界**:#28 是行程頁的 `.chk` 24×24px(Bar 2026-09-10 裁定暫不處理);本項是**記帳表單**這一整批,不同畫面、不同使用情境 —— 記帳多在店內站著單手操作,錯按成本是寫錯一筆帳。
    - **`.ledger-participant-choice` 28px 最該優先**:它是分攤成員的多選 chip,一次要點好幾個,而且 v123 才剛把它的選取底色主題化(表示它是活的、會被用到的元件)。
    - 改動會撐開既有版面(`.ledger-item-control-row` 的 grid 欄寬、popover 高度),需逐一評估。屬 Tier 2,需 forward bump。
    - **⚠ 2026-09-23 實機走查修正**:上列數字全部是 **CSS 宣告值**,不等於使用者實際碰得到的尺寸。在 active-trip 下實際打開「新增消費」表單量測,**該畫面最小的控制項是 40px**(`個人／團體`、`JPY／TWD` 四顆 segmented 選項)與 **36px** 的「多品項」toggle —— **28–36px 那批多數在摺疊區內**,預設收合。
      - `.ledger-participant-choice`(28px)位於「分攤成員」展開區,預設顯示為「全員 4 人 ⌄」一行收合。**該展開狀態尚未實測**(嘗試展開時誤觸關閉),動工前要先看過真實展開的樣子再決定要不要改。
      - 因此本項的**實際影響面比原記述小**,優先序可往後調;但「分攤成員一次要點好幾個」的論點若在展開後成立,仍值得處理。
    - **不要動的部分 —— 新增消費表單的金額欄是好範例**:實測 `金額 (¥)` 輸入框 **67px 高、30px 字級**,`明細` 46px／16px,表單總高 694px(視窗 812)不需捲動即可看到儲存鈕,次要資訊收在「其他資訊(選填)· 今天 · 餐飲 · 現金 · 無備註 ⌄」一行摺疊。**這正是站在店裡單手輸入該有的樣子**,調整周邊控制項時不得為了「一致」把它縮小。
    - 來源:2026-09-23 新模型全專案複審(CSS 量測)+ 同日 active-trip 實機走查(渲染量測)。

42. **`ubuntu-latest` 將於 2026-10-19 起移轉到 Ubuntu 26**:`.github/workflows/qa.yml` 的兩個 job(`sanity` L24、`browser-qa` L45)都用 `runs-on: ubuntu-latest`。GitHub 已在 CI log 以 annotation 預告(`actions/runner-images#14748`)。

    - **不是現在要改**:移轉是 GitHub 端自動發生,`ubuntu-latest` 這個標籤本身不需要動。本項的用途是**留一個已知的變因**,日後 CI 若出現「程式沒改卻突然紅」,先來看這裡而不是從頭 debug。
    - **本專案的實際暴露面**:`browser-qa` 跑 `npx playwright install --with-deps chromium` —— **系統層相依由 runner 的 OS 版本決定**,這是最可能受影響的一步。`@playwright/test` 目前固定在 `1.62.0`,若該版的 `--with-deps` 不支援 Ubuntu 26 的套件名,安裝會失敗。
    - **actions 版本無虞**:`checkout@v7` 與 `setup-node@v7` 已於 backlog #26(2026-09-10)升到當時最新,runtime 為 node24。`node-version: '20'` 是 App 端測試用的 Node,與 runner OS 無關。
    - **若真的出事**:短期釘住 `runs-on: ubuntu-24.04` 即可止血(GitHub 保證舊標籤在移轉後仍可用一段時間),再從容升 Playwright。**不要在沒壞之前先釘版** —— 釘住反而會錯過日後的安全性更新。
    - 來源:2026-09-23 v126 的 CI log annotation。

44. **下一站卡片的三處呈現問題**(2026-09-23 active-trip 實測):

    - **兩顆決策鈕都被截斷,且無 `aria-label`**:`完成：廣島和平紀念資料館` 寬 160px、`跳過：廣島和平紀念資料館` 寬 120px,`scrollWidth > clientWidth` 皆為 true,畫面上是「完成：廣島和平…」「跳過：廣…」。站名在上一行「目前 10:10 廣島和平紀念資料館」已經寫過,**按鈕重複一次又截斷,資訊沒增加、可讀性反而更差**。最省事的修法是按鈕只留「完成」「跳過」。
    - **「目前」一詞兩義**:卡片寫 `9:00 - 11:30`、`目前 10:10 廣島和平紀念資料館`,而當下時鐘是 11:00。這裡的「目前」指**該站的排定時間**,不是「現在幾點」。
    - **剩餘站數要心算**:`4 站` 與 `2 站已自動略過` 兩個 chip 並排,沒有直接講還剩幾站。走在路上要自己減。
    - 屬 Tier 2,需 forward bump。

45. **v125 的副作用:兩顆「新增」變成兩種橘**:採買清單空狀態裡,右上 ＋ FAB 與中間「新增第一項採買」並排,實測 computed background —— `.shopping-add-button`(吃 `--coral`)`rgb(223,95,58)`,`.btn.cta`(吃 `--t-cta-bg`)`rgb(192,82,50)`。

    - **成因是 v125 刻意的**:CTA 為了讓白字過 AA 而由 accent 壓深一階(海洋 `#df5f3a → #c05232`),FAB 仍吃原本的 accent。同一畫面、同一個「新增」動作、差一階深淺。
    - **兩個方向**:(a) 圓形圖示鈕也改吃 `--action-cta-bg`;(b) 認定 FAB 屬「accent 裝飾層」而非動作角色,維持現狀並在 `04_UI_GUIDELINES` 寫明。**不要只改一半**。
    - 只影響海洋與象牙兩組主題(其餘四組 `--t-cta-bg` 等於 accent,兩者本來就同色)。

46. **行程頁的「顯示全部／隱藏已完成」應改為 segmented control**:兩顆等寬 `.btn` 並排表達互斥狀態,當前狀態那顆填深色 —— 但依 2026-09-12 核定的形狀語意,**膠囊＝狀態、圓角矩形＝動作**,這裡是狀態卻用了動作的形狀。

    - **同一個 App 裡已有正確做法**:採買清單的「待買／已買」就是 `.shopping-list-segment`。兩種做法並存。
    - active-trip 下該列還會多一顆「⟳ 回到現在」,三個控制擠在一列,更需要把前兩顆收成一組。

47. **航班時刻字串在 375px 斷在括號中間**:行程頁 Day 1 的 9:00 卡片,資訊框顯示為 `9:00開櫃 11:30(TPE台灣桃` / `園)-15:05(OKJ日本岡山)`,`桃園` 被拆開。可用 `overflow-wrap`／`word-break` 或在資料層加不斷行標記處理。出發前與 active-trip 狀態皆重現。

48. **出發前的今天頁文案自相矛盾**:hero 先寫「今天沒有排定行程 🗓️／出發當天這裡會自動顯示當日安排與下一站。」,緊接著就列出 `9:00 桃園機場第一航廈1樓`、`15:05 ORIX租車 岡山機場`、`還有 4 站`。那是 Day 1 的灰階預覽,但**文案沒說那是哪一天**,讀起來是「沒行程」然後看到行程。缺一個像「出發當天(10/18)的安排」的標頭。另外「還有 4 站,查看完整行程」(純文字)與「查看完整行程 →」(按鈕)上下相鄰,同一件事講兩次。

49. **購物頁的資訊密度**:(a) 營業時間擠成一行 —— `商場10:00-21:00 / 餐廳11:00-22:00 / 美食廣場10:00-21:00 / 超市07:00-23:00`,站在門口想確認超市幾點關要從頭讀到尾;(b) 永旺夢樂城有 **11 個樓層列**,每列 45px、**合計 495px**,在 375×812 上光摺疊標題就超過一個半螢幕,而 5F／6F 各只有 **1 家**。可考慮把「1 家」的樓層併入上一層或改為單行列出。

## 想法池(未承諾)
- 社群內容抓取(Facebook 等)——需 Firecrawl/Playwright MCP,尚未配置
