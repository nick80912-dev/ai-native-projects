...
43. **【BUG】同一天的進度數字,三個畫面互相矛盾**(2026-09-23 於 v126 實測,時間模擬 Day 2／10-19 11:00):

    | 畫面 | 顯示 |
    |---|---|
    | 今天頁 hero | **1 / 8** |
    | 行程頁 day-head | 完成 **0**・略過 3 ／共 **14** |
    | 行程頁實際渲染 | **11** 張 `.item`,`.item.checked` **0** 張 |

    - **使用者看得到的矛盾**:今天頁說「已完成 1」,切到行程頁變成「完成 0」。總數則有 8／14／11 三個版本。
    - **不是打磨,是資料呈現錯誤**:兩處計數各自推導、分母定義不同(疑似 hero 以「串點視為一站」計、day-head 計所有項目、渲染又受篩選影響),但**使用者沒有線索知道它們在數不同的東西**。
    - **動工前先釐清三個分母各自的定義**,再決定是統一口徑還是各自標示清楚(例如 hero 改寫成「主要行程 1/8」)。**先別急著改數字** —— #31／#37 的前車之鑑是「看起來像疏漏就去修」而被測試擋下。
    - 屬 Tier 2,需 forward bump。來源:2026-09-23 新模型 active-trip 實機走查。
