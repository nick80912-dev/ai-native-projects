...
> **編號刻意不連續,不得重排**:`tasks/current.md`、`tasks/done.md` 與 `07_CHANGELOG.md` 都以編號互相引用,重排會打斷既有交叉引用。已歸檔項目的編號一律**留空不回收**(目前缺號:1、3b、5、6–9、13–19、21);新項目接在現有最大號之後。
...
10. **受保護紀錄 Tag 文案改為「已鎖帳」**(Bar 已裁定 2026-07-30):團體帳受保護紀錄的 badge 由「還款確認後保護」縮短為「已鎖帳」(`index.html` `renderLedgerRecentRecord`,現行唯一出現處)。Tag 只負責快速辨識,完整原因移交明細頁,新增說明文字「此筆消費已完成還款確認,目前已鎖帳,無法再編輯或刪除。」(注意:明細頁目前只有「查看不可改寫歷史」按鈕,沒有任何說明句,這句是**新增**不是搬移)。現行無測試斷言此字串,交付時需補一則。**嚴格限制**:只改顯示文案與對應測試,不得變更 `_correctionProtected` 判定、編輯／刪除權限、還款確認流程或不可改寫歷史;`assertCanEditLedgerRecord` / `assertCanDeleteLedgerRecord` 的錯誤訊息屬於行為契約,本項不動。備選字「已鎖定」已評估後不採用(缺帳務情境)。
