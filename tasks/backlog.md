...
32. **多品項新增消費:帳單摘要列與單品項對齊(搭便車項目,與 #30／#31 同批)**:多品項模式帳單資訊卡裡的 `.ledger-multi-summary`(收合時顯示 `2026/09/12 · 現金 · 餐飲`,343×44)與單品項的同一個邏輯控制項有四處不一致。2026-09-12 在正式站 v115、390×844 實測。
    ...
    - **一併記錄但不建議處理**:初始捲動位置下,`稅與優惠券(選填)` 那一列(高 44px)被 sticky 的 `.ledger-sheet-actions`(z-index 2,不透明)蓋住 **24px**,`elementFromPoint` 在該列中心回傳的是操作列。**但捲到底後重疊為 0、點擊正確落回該列**,而該 sheet 總捲動量僅 111px。**屬發現率問題,不是 v98 那類阻斷式遮罩**。若 #33(移除取消按鈕)成案,操作列由 172px 降至約 116px,此項自動消失。

    - **為何不單獨處理**:同 #30／#31 —— 依 ADR 0019 需完整 forward bump。**三項同批執行**。

33. **記帳 sheet 操作列簡化:移除「取消」按鈕(Bar 2026-09-12 提案,待裁定)**:提案內容為刪除底部的「取消」,離開改由右上角 `×` 承擔,理由是底部三顆全寬按鈕過高、視覺過重。2026-09-12 於正式站 v115、390×844 實測評估。
    ...
    - **建議做法**:若要執行,**與「點背景關閉」配套** —— 給 `.ledger-sheet-overlay` 加上關閉行為。如此失去一顆底部按鈕、卻換到一個比按鈕大得多的離開區域,可及性不降反升,也是 bottom sheet 的常規行為。**單獨移除而不配套,不建議**。

    - **待裁定事項**:(1) 是否執行;(2) 若執行,是否同時加入點背景關閉;(3) 是否一併補未存內容確認(此項可獨立於本案)。
