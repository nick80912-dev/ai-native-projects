...
44. **下一站卡片的兩處呈現問題**(2026-09-23 active-trip 實測;原三項中的「決策鈕截斷」已隨 **v128** 修正):

    - ~~兩顆決策鈕都被截斷,且無 `aria-label`~~ → **v128 已修**:可見文字只留「完成／跳過」(與非串點卡片一致),完整站名移到 `aria-label`。站名本來就無條件顯示在按鈕正上方的「目前 …」那行。實測 375px 下 `scrollWidth > clientWidth` 皆為 false。
