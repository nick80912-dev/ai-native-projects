...
50. **税抜 帳編輯／更正後,換算幣別在品項間最多挪動 ±5**:v135 修掉「每編輯一次就再加一次稅」之後,總額與輸入幣別的每個品項都已精確不變(1,500 例隨機);殘留的是**換算幣別**的品項間分配。成因:`calculateMultiItemAmounts()` 以原始未稅權重(含 basis points)分配換算幣別,這組權重沒有存進紀錄,重載時只能用含稅金額當權重,四捨五入落點因此不同。
    ...
    - **可能的根治**:換算幣別改依「輸入幣別的品項最終金額」分配,而非原始權重 —— 新增與重載用的就是同一組權重,來回必然一致。代價是新帳的換算幣別分配與 v134 以前可能差 ±1,既有測試需同步更新;已存在的紀錄不重算。
    - 來源:2026-09-24 分帳稽核(v135)的隨機來回測試。
    - **2026-09-24 原型實測(未落地)**:在真實 App 內暫時把換算幣別改為依「輸入幣別的品項最終金額」分配 ——
      - 編輯不改直接存:隨機 3,000 例,**兩幣逐品項 0 變動**、總額 0 變動(根治成立)。
      - 新帳與 v135 相比:3,000 筆中 501 筆(約 17%)換算幣別的品項間分配不同,最多 ±4;**輸入幣別逐品項與總額全部相同**。
      - 分佈:單品項 0%;多品項 JPY 輸入 税込無折扣 0／672;JPY 輸入 税抜或有折扣約 2–22%;**TWD 輸入**除税込無折扣外約 60–95%(台幣金額小,當權重較粗)。
