...
const { appVersion, swVersion, shellPath } = require('./support/version');
...
assert.ok(index.includes('<script src="' + shellPath('app-version.js') + '"></script>'), 'current document loads the immutable generation version');
assert.match(index, new RegExp('<script src="' + shellPath('app-version.js').replace(/[./]/g, '\\$&') + '"></script>\\s*' + '<script id="builtinSnapshotMarker">var BUILTIN_HTML_VERSION=\'v\\d+\';var BUILTIN_HTML_TS=\\d+;</script>\\s*' + '<script src="' + shellPath('builtin-snapshot.js').replace(/[./]/g, '\\$&') + '"></script>'), 'runtime marker and generated BUILTIN use immutable generation paths before boot');
...
assert.ok(serviceWorker.includes("'./" + shellPath('index.html') + "'"), 'App Shell caches the immutable current document');
assert.ok(serviceWorker.includes("'./" + shellPath('app-version.js') + "'"), 'App Shell caches the immutable version file');
assert.ok(serviceWorker.includes("'./" + shellPath('builtin-snapshot.js') + "'"), 'App Shell caches the immutable generated BUILTIN asset');
...
assert.ok(netlify.includes('for = "/' + shellPath('app-version.js') + '"'), 'Netlify pins the immutable current version file header');
assert.ok(netlify.includes('for = "/' + shellPath('builtin-snapshot.js') + '"'), 'Netlify prevents a stale generated BUILTIN asset from mixing with new HTML');
