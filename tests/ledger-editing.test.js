...
assert.strictEqual(single[0].member,'Bar','編輯墓碑記錄目前操作者');
assert.strictEqual(single[1].member,old.member,'replacement 精確保留原付款人');

const hostileReplacement=Object.assign({},replacement,{member:'Amy'});
const immutableMember=plain(mod.buildSharedLedgerEditBatch([old],[hostileReplacement],{member:'Bar',now:1784428800500,random(){return 0.3;}}));
assert.strictEqual(immutableMember[1].member,'Bar','草稿帶入不同 member 仍不得改變原付款人');

const spacedOwner=Object.assign({},old,{id:'old-spaced',member:'　Bar　'});
const spacedEdit=plain(mod.buildSharedLedgerEditBatch([spacedOwner],[Object.assign({},replacement,{member:'Bar'})],{member:'Bar',now:1784428800600,random(){return 0.35;}}));
assert.strictEqual(spacedEdit[1].member,'　Bar　','canonical 比對只作授權 key,replacement 保留原付款人字串');
assert.throws(
  function(){mod.buildSharedLedgerEditBatch([old],[replacement],{member:'Amy',now:1784428800700,random(){return 0.4;}});},
  /僅付款人可編輯/,
  '非付款人無法直接建立團體編輯批次'
);
...
assert(batch.slice(2).every(record=>record.member===old.member),'同批 replacement 全部保留原付款人');
assert.throws(
  function(){
    const mixedOwners=[old,Object.assign({},old,{id:'old-other',member:'Amy',batchId:'batch-old'})];
    mod.buildSharedLedgerEditBatch(mixedOwners,newItems,{member:'Bar',now:1784428801500,random(){return 0.55;}});
  },
  /僅付款人可編輯|同批紀錄付款人不一致/,
  '同批混入其他付款人時 fail-closed 整批拒絕'
);
