...
- `ledger-225.test.js`：驗證 Ledger 日期群組、顯示單元、選取／批次展開與最近消費 renderer；v98 後續鎖定個人／團體近期消費卡嚴格兩行、第一行「品項／付款方式／代購或付款分攤」、第二行「店家／類別／免稅／TEST／待同步／鎖帳或更正」的精確順序、狀態配色 class、鎖帳／更正互斥及缺欄位安全降級。執行：`node tests/ledger-225.test.js`。
...
- `browser/data-observability.spec.js`：以 320／375／390px 驗證個人／團體近期消費卡只有兩個 single-line content row、長品項與店家採 ellipsis、卡片高度緊湊、文字不與金額重疊；同時鎖定指定欄位順序、團體付款分攤與鎖帳的中性色一致、待同步保留黃色，以及健康同步 header、partial 來源與設定頁資料健康摘要。執行：`npx playwright test tests/browser/data-observability.spec.js`。
- `browser/proxy-inline.spec.js`：以長品名、長代購姓名驗證最近消費、完整紀錄、批次展開子項與採買卡均顯示「幫 [姓名] 買」；近期消費在 320／375／390px 嚴格維持兩行截斷及 compact 字級，Shopping 卡維持原樣，兩者皆不與金額／操作欄重疊且無水平 overflow。執行：`npx playwright test tests/browser/proxy-inline.spec.js`。
