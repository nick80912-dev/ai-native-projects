...
- `shopping-ledger-links.test.js`:採買清單 ↔ Ledger 持久關聯。涵蓋 Shopping Item normalizer 的 `completedAt`／`splitGroupId`／`ledgerLinks` 補欄與拒絕規則、append-only link 與 `releasedAt` 解除語意、三態動態推導(個人／團體、queue／bridge、replacement、confirmed tombstone、universe 不符、資料未就緒一律 `unverified`)、多品項 source→record 一一對應與交握錯誤整批不回寫、`applyLedgerLinks` 原子回寫、已買多選 preflight 整批阻擋、部分購買拆分(原 ID 為已買、剩餘插在正後方、共用 `splitGroupId`、失敗完全不寫入)、個人狀態備份 v5 與未知版本拒絕,以及「來源 IDs 不得進入 Ledger record」等原始碼契約。執行:`node tests/shopping-ledger-links.test.js`。
