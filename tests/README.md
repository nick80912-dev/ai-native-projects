...
- `ledger-225.test.js`：驗證 Ledger 日期群組、顯示單元、選取／批次展開與最近消費 renderer；v98 後續鎖定團體付款／分攤資訊與品項同行、個人／團體類別接在店家後、支付方式獨立，以及缺店家／缺支付方式的安全降級。執行：`node tests/ledger-225.test.js`。
...
- `browser/data-observability.spec.js`：以 320／375／390px 驗證 v88 個人／團體今日支出與旅程累計不溢位；另鎖定近期消費卡的團體付款／分攤資訊與品項同行、個人／團體「店家 · 類別」順序、支付方式獨立，以及健康同步 header、partial 來源與設定頁資料健康摘要。執行：`npx playwright test tests/browser/data-observability.spec.js`。
