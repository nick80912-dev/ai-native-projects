...
- `shopping-list.test.js`:採買清單 store 正規化與封閉分類（代購不再是分類）、多對象新增與共用名單、每人／總數摘要、`儲存並新增` 保留規則、姓名-only 代購 badge／淡金類別 badge／繁中文字型、總需求大於 1 的部分購買 eligibility、卡片與明細事件邊界、完成與批次 selection 分離、0 項提示與單行三按鈕工具列、底部 spacer、Today 提醒歸組、逐 allocation Buy-to-Ledger 單筆與多品項 prefill、Scroll-only CSS 契約；另涵蓋**站點排序**(`dayIndex → 當日 items index` 的單一排名來源、待買頁與 Today 共用、同站點內維持 store order、穩定排序、不重寫本機順序、已買頁排除在外)與**孤兒 `stopRef` 三態**(`tripDatasetAuthority` 只認 `online` 快照、`builtin`／`legacy-migrated`／無快照一律降級待確認、確認失效的警告文案、編輯表單以原值為選中 option 且不自動清空)。執行:`node tests/shopping-list.test.js`。
