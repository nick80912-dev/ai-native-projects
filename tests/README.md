...
- `builtin-snapshot.test.js`：直接執行正式 `index.html` 最終生效的 BUILTIN 與既有 runtime parser，鎖定岡山 Day 1–6、非空 places／restaurants／shopping、TripConfig 八 key 各一次、無東京／新宿，以及 Ledger 僅有 schema 推導的 21 欄空 header。執行：`node tests/builtin-snapshot.test.js`。
- `builtin-snapshot-refresh.test.js`：驗證 BUILTIN 刷新工具不請求 live Ledger、schema primary／alias header、preview no-write、明確 `--write`、原子替換、legacy mutation 移除、無漂移與來源／驗證失敗保留原檔。測試只使用本機 fixtures，不連 Google。執行：`node tests/builtin-snapshot-refresh.test.js`。
