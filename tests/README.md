...
- `ledger-entry-p0.test.js`／`ledger-three-second-entry.test.js`:驗證單品項新增消費的主流程階層、`其他資訊（選填）` 摘要與日期標籤、代購欄位位置、個人／團體鍵盤 Next 聚焦，以及 Enter 不直接送出；保留既有儲存 guard。執行:`node tests/ledger-entry-p0.test.js`、`node tests/ledger-three-second-entry.test.js`。
- `shopping-ledger-links.test.js`:採買清單 ↔ Ledger 持久關聯。涵蓋 v1～v7 Shopping Item → `allocations[]` 正規化、allocation-level append-only `ledgerLinks`／`releasedAt`、個人／團體三態與 item 聚合狀態（`未記帳`／`記帳 2／3`／`已記帳`／`狀態待確認`）、queue／bridge／replacement／confirmed tombstone／universe、逐 allocation source→record 一一對應與原子回寫、已買多選 preflight、逐人部分購買與 split-group 原需求重建、部分購買卡片入口契約、明細的狀態感知需求文案、allocation 編輯鎖定、刪除警告人數、個人狀態備份 v7 round trip／v1～v6 還原／未知版本拒絕，以及來源 item／allocation IDs 不得進入 Ledger 21 欄。執行:`node tests/shopping-ledger-links.test.js`。
- `shopping-list.test.js`:採買清單 store 正規化與封閉分類（代購不再是分類）、多對象新增與共用名單、每人／總數摘要、`儲存並新增` 保留規則、姓名-only 代購 badge／淡金類別 badge、總需求大於 1 的部分購買 eligibility、卡片與明細事件邊界、完成與批次 selection 分離、0 項提示與單行三按鈕工具列、底部 spacer、Today 提醒歸組、逐 allocation Buy-to-Ledger 單筆與多品項 prefill、Scroll-only CSS 契約；另涵蓋新建／連續新增的 `1 個` 預設、無空白／「不指定」單位、`個` 的設定頁刪除保護、既有非空單位保留與空單位只在草稿預選，以及**站點排序**(`dayIndex → 當日 items index`)與**孤兒 `stopRef` 三態**。執行:`node tests/shopping-list.test.js`。
