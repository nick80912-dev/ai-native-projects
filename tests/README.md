...
- `browser/shopping-photo.spec.js`:以 Chromium 真實 Canvas／IndexedDB 驗證採買單張照片選取、壓縮、重載持久化、卡片只顯示無文字迴紋針、詳情全畫面查看、備份排除、移除最後引用清 Blob，以及 320／375／390px 水平 overflow、console error／pageerror 為 0。執行:`npx playwright test tests/browser/shopping-photo.spec.js`。
