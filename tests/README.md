...
- `ledger-ui-state-wiring.test.js`／`browser/ledger-ui-state.spec.js`：驗證 production handler 全數透過 workflow dispatch、module 納入頁面與離線 App Shell、session-only 狀態不進備份；Browser 涵蓋 filter panel DOM／焦點連續性、搜尋／分組／清除、多選／全選、關閉與切軌重設，以及 320／375／390px 無水平 overflow。執行：`node tests/ledger-ui-state-wiring.test.js`、`npx playwright test tests/browser/ledger-ui-state.spec.js`。
