...
- `shopping-list.test.js`:採買清單 store 正規化與封閉分類（代購不再是分類）、多對象新增與共用名單、每人／總數摘要、`儲存並新增` 保留規則、姓名-only 代購 badge／淡金類別 badge、總需求大於 1 的部分購買 eligibility、卡片與明細事件邊界、完成與批次 selection 分離、0 項提示與單行三按鈕工具列、底部 spacer、Today 提醒歸組、逐 allocation Buy-to-Ledger 單筆與多品項 prefill、Scroll-only CSS 契約；另涵蓋新建／連續新增的 `1 個` 預設、無空白／「不指定」單位、`個` 的設定頁刪除保護、既有非空單位保留與空單位只在草稿預選、**站點排序**(`dayIndex → 當日 items index`)與**孤兒 `stopRef` 三態**，以及 pending／done 明確 page context：四類待買卡與多選模式不輸出地點 DOM、已買卡保留地點。執行:`node tests/shopping-list.test.js`。
