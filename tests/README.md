...
- `ledger-settlement-correction.test.js`：結算後不可改寫與引導式更正總測試。涵蓋 claim 建立位置保護切點、正式／TEST 隔離、confirm 復原、多人收據、永久保護、完整版本投影、commit-last 缺件 inert、重複更正、整張作廢、跨裝置 canonical／losing conflict、不合法事件 fail-closed、付款人／原因守門、刪改阻擋、action model，以及已全數還款後更正產生的新餘額差。執行：`node tests/ledger-settlement-correction.test.js`。
- `ledger-list-actions.test.js`：除清單卡片、明細與操作選單外，驗證受保護收據只顯示「更正收據」、引導式更正 Sheet／預覽入口、更正次數 badge、不可改寫歷史入口，以及完整紀錄保留作廢收據歷史。執行：`node tests/ledger-list-actions.test.js`。
