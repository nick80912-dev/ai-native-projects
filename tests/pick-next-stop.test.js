...
/* ---- 情境7:父列是第一站，controller 必須等所有實際站點清除後才完成 ---- */
{
  const sb = makeSandbox();
  const items = [
    { id:'parent', time:'17:30', act:'機場與取車', place:'岡山桃太郎機場', ref:'P001' },
    { id:'child', time:'19:30', act:'', place:'ORIX租車 岡山機場店', ref:'P048' },
    { id:'next', time:'22:00', act:'住宿 check in', place:'Guest House', ref:'P002' }
  ];
  const cluster = sb.getChildStopCluster(items, items[0]);
  assert(cluster, '父列第一站加一個後續子站即可成立父子卡');
  assert.deepStrictEqual(Array.from(cluster.items).map(function(item){ return item.id; }), ['parent','child']);
  ...
  assert.strictEqual(
    ...
    false,
    '完成第一站不會完成整組 controller'
  );
  let childPick = sb.pickClusterChild(cluster, progress, sb.getChecks(), 19*60, { day, dayIndex });
  ...
  progress = sb.getDayProgress(day, dayIndex);
  assert.strictEqual(!!progress.done['parent__cluster'], true, '全部站點完成後 controller 才完成');

  sb.setItemCompletion(day, dayIndex, 'child', 'clear');
  progress = sb.getDayProgress(day, dayIndex);
  assert.strictEqual(sb.reconcileClusterController(day, dayIndex, cluster, progress, sb.getChecks()), true);
  progress = sb.getDayProgress(day, dayIndex);
  assert.strictEqual(!!progress.done['parent__cluster'], false, '取消任一站完成會重新開啟 controller');
}

/* ---- 父列沒有地點或 ID 時仍需兩個後續子站 ---- */
{
  const sb = makeSandbox();
  const noPlace = [
