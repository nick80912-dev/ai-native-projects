...
(async function(){
  ...
  assert.strictEqual((sheetSource.match(/preventDefault\(\)/g)||[]).length,6,'preventDefault is limited to the five single/multi Enter handlers and calculator Tab containment, not touch gestures');
...
(async function(){
  ...
  const blockedResult=await saveSandbox.saveLedgerEntry(false);
  assert.deepStrictEqual(JSON.parse(JSON.stringify(blockedResult)),{ok:false,pending:true},'a second tap or keyboard Done is ignored while confirmation is pending');
  assert.strictEqual(buildCalls,1,'the pending guard prevents a second record ID from being prepared');
