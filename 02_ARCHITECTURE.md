...
`shell/v111/builtin-snapshot.js` 是 Tier 3 generated asset，唯一內容 authority 是 `tools/refresh-builtin-snapshot.js`。v111 HTML 只保留版本／timestamp marker 與安全 boot guard，不複製資料 payload。asset 缺失或 `BUILTIN_ASSET_VERSION !== APP_VERSION` 時，App 只接受已通過完整 shape／validation 的 local active／previous snapshot；若本機也沒有有效資料，顯示可重新載入與複製診斷的復原頁，且不建立空 DB、不啟動背景同步。SW 以同一版本 cache generation 將 immutable HTML、App version 與 asset 一起納入 App Shell；root v110 bridge 則保留 predecessor 的 inline BUILTIN，僅用於 v111 安裝前或安裝失敗。
