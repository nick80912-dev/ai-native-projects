...
| `shell/v114/index.html`／`app-version.js` | current App 原始碼與版本 identity；改壞直接影響成功升級的線上使用者 |
...
`sw.js`、`shell/v114/index.html`／`app-version.js`／`builtin-snapshot.js`、root v110 bridge 與 `netlify.toml` 對應 cache header **視為同一風險群組**,理由:它們共同決定「使用者裝置上會不會原子取得新版」。因此:
...
| `shell/v114/builtin-snapshot.js` | Google Sheets 七張非 Ledger 發布 CSV + `schema.js` Ledger header + App version | 先執行 preview，再以 `node tools/refresh-builtin-snapshot.js --write` 產生；禁止手改或在 v114 HTML 複製 payload |
