...
> 適用 runtime:`feat/settings-grouped-v74`,SW/App **v74**。**尚未 merge `dev`,尚未部署。**
...
> **需 Bar 裁定的一項觀察**:測試模式警告列的標題文字沿用既有警告語意色 `var(--coral)`(規格 §3.4 要求),實測對比為 **ocean 3.54／ivory 3.68／mist 3.65／cedar 3.80／wisteria 4.97／tea 4.86**,在四個主題下低於 WCAG AA 對一般文字的 4.5。此為 App 既有的 `--coral` 語意色水準(既有 `.settings-warning-tag` 同色),非本次新引入;且該列同時有 ⚠ 符號與 `前往關閉` 次要文字(對比 5.43–7.77),訊息不單靠顏色傳達。若要提高對比需改動主題 token,已超出本次規格範圍,故未自行變更。
