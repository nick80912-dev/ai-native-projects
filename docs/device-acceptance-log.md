...
>
> **本節的桌機預檢由 AI 於 2026-09-10 完成,`Bar 真機` 欄一律留空 —— G1 是 Bar 專屬職責,不得因自動驗證全綠而代勾。**

## Z1. 三組主題彼此可分辨(本批的目的)

| ID | 驗收項目 | 桌機預檢(2026-09-10) | Bar 真機 |
|---|---|---|---|
| Z1-a | 三組主題的**頁面底色**在同一支手機上看得出差別 | 🖥️ paper RGB 距離:杉綠↔霧藍 **12**、杉綠↔焙茶 **12**、霧藍↔焙茶 **20**(契約門檻 ≥10)· 📱 **iPhone 必測** | ☐ |
| Z1-b | 三組主題的**強調色**分屬不同色系 | 🖥️ 杉綠 `#bd4e24` 鳥居橘／霧藍 `#9a6614` 日光金／焙茶 `#405c7a` 倉敷靛藍 | ☐ |
| Z1-c | 三組主題的**次要色**互不相同,且都不是舊的共用紫 `#7659a0` | 🖥️ 杉綠 `#6f6532`／霧藍 `#4b746c`／焙茶 `#9a4f30`,三色互異 | ☐ |
| Z1-d | 主操作色維持既有值:杉綠 `#2f6b4f`、焙茶 `#896748` | 🖥️ computed value 核對相符 | ☐ |
| Z1-e | 在設定→主題頁連續切換三組,每次切換畫面確實跟著變,不需重新啟動 | 🧪 `applyTheme()` 桌機逐一切換皆即時生效 · 📱 **iPhone 必測**(走真實的設定頁操作路徑) | ☐ |
...
| ID | 驗收項目 | 桌機預檢(2026-09-10) | Bar 真機 |
|---|---|---|---|
| Z2-a | 三組主題下主要文字、次要文字、主操作色對底色皆 ≥ WCAG AA 4.5:1 | 🖥️ 全數通過,最低為**焙茶的操作色 4.53** | ☐ |
| Z2-b | 深色 chrome 上的白字清楚可讀 | 🖥️ 杉綠 12.23／霧藍 8.85／焙茶 11.38 | ☐ |
| Z2-c | 320／375／390px × 今天／行程／購物／分帳,水平溢位為 0 | 🖥️ 36 組組合實測**全部為 0** | ☐ |
| Z2-d | 三組主題下 console error 與 pageerror 均為 0 | 🖥️ 皆為 0;`healthCheck()` 回傳空陣列 | ☐ |
...
| ID | 驗收項目 | 桌機預檢(2026-09-10) | Bar 真機 |
|---|---|---|---|
| Z3-a | Ocean／Ivory／Wisteria 三組主題外觀與 v112 相同 | 🖥️ 三組的 13 個 token 與 v112 逐一比對無差異 | ☐ |
| Z3-b | 主題選擇會被記住:切換後關閉 PWA、重新開啟仍是同一組 | 📱 **iPhone 必測**(冷啟動與 localStorage 保存桌機不足採信) | ☐ |
| Z3-c | 個人備份匯出→還原後主題設定跟著回來 | 📱 **iPhone 必測** | ☐ |
| Z3-d | 版本資訊頁顯示 **v113** | 🖥️ 線上 `sw.js` 與 `shell/v113/app-version.js` 皆 v113 · 📱 **iPhone 必測** | ☐ |
| Z3-e | 離線(飛航模式)冷啟動正常,不空白頁 | 📱 **iPhone 必測** | ☐ |
