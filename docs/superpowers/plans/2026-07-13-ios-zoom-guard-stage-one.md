...
- [x] Write a Node test that extracts the style block and asserts the global `input,select,textarea` rule exists and known form selectors contain no font size below 16px.
- [x] Run `node tests/ios-zoom-guard.test.js`; expect failure because existing rules are 14px/15px and the global floor is absent.
- [x] Add `input,select,textarea{font-size:16px}` and raise `.inline-add input`, `.field input,.field select`, `.shop-search input`, and diagnostic form input rules to 16px.
...
- [x] Extend the test to extract the new helpers and assert: form focus with scale > 1 temporarily adds `maximum-scale=1`; the timer restores the exact original string; gesture-origin zoom does nothing; missing `visualViewport` does nothing.
- [x] Run the focused test; expect failure because helpers do not exist.
- [x] Add `VIEWPORT_ORIGINAL` from the existing meta content and minimal ES5 helpers. Extend the existing setup handler with `focusin`, multi-touch/gesture tracking, and `focusout`; do not create a second focusout handler.
...
- [x] Extend the test to require the CSS rule, retained JS guard, absence of persistent viewport restrictions, and SW v4.
- [x] Run the test; expect failure on missing CSS/cache changes.
- [x] Add the CSS rule and change only `CACHE_NAME` from v3 to v4.
- [x] Rerun the focused test; expect PASS.
...
- [x] Document the 16px form minimum, gesture policy, transient viewport lock, and stage-one validation status.
- [x] Add the viewport accessibility invariant to the manifest and a stage-one changelog entry.
