...
function project(entry){
  ...
  if(String(entry&&entry.category||'')==='sync'&&message==='ledger 增量讀取失敗,維持 CSV 路徑:ledger 增量讀取逾時')return {severity:'degraded',title:'分帳快速同步暫時逾時',impact:'目前仍可使用',fallback:'已改用一般 CSV 同步',raw:message};
...
The Ledger timeout specialization requires both the `sync` category and the complete producer-emitted message above. HTTP 403／500, fetch／network errors, appended timeout text, spelling near-matches, and identical wording under another category retain the conservative raw-title／empty-fallback projection; tests lock each negative case so the broad pre-review classifier cannot return.
