...
storeName, replacesRecordId
```

後八欄皆為選填；舊 payload 未提供時，Apps Script 會寫入空字串。`participants` 必須是 JSON Array 字串，不得使用逗號字串。

Ledger 2.7 寫入範例：

```json
{"id":"1784428800000-test","time":"2026-07-19T08:00:00.000Z","member":"Bar","category":"交通","detail":"[TEST] Ledger 2.7 contract","amountJpy":500,"amountTwd":100,"note":"","participants":"[\"Bar\",\"Amy\"]","payMethod":"現金","recordType":"expense","targetRecordId":"","deleteReason":"","batchId":"batch-test-001","storeName":"岡山站","replacesRecordId":""}
```
...
1. Bar 先在 Google Sheet「分帳紀錄」既有 14 欄末端新增：`店名`、`取代紀錄ID`。
...
Apps Script 部署完成並通過下列真實驗收後，才可交付使用 Schema 2.7 的前端契約。
...
3. 以新 ID POST 一筆含 `storeName`、`replacesRecordId` 的 Ledger 2.7 `[TEST]` 分帳，確認回 `{ok:true}`。
4. 重送相同 ID，確認回 `{ok:true,dup:true}`。
5. 等待分帳 CSV，確認該 ID 只出現一列，且 `participants` JSON 字串、支付方式、紀錄類型、批次 ID、店名與取代紀錄 ID 正確落位；既有 14 欄沒有位移。測試資料由 Bar 驗收後手動清除。

## 回滾

在 Apps Script「管理部署」把 Web App 指回上一個已知正常版本，保持同一 `/exec` URL。Schema 2.7 Apps Script 向後相容 14 欄 payload，通常可保留；若仍需回退且前端 Schema 2.7 已發佈，應同時依專案回滾程序回復對應 Netlify 部署並再次 bump Service Worker cache，不可只回退其中一側而留下不相容契約。
