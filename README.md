...
## 專案檔案
- `index.html` / `app-version.js` — 保留正式站前一代 v110 bridge；讓尚未更新的 v110 worker 在 v113 安裝失敗時仍可運作
- `shell/v113/index.html` / `shell/v113/app-version.js` — v113 不可變文件與 App runtime 版本來源；成功啟用的 `sw.js` 才接管 root 導覽
- `shell/v113/builtin-snapshot.js` — 由刷新工具產生的版本綁定離線資料資產；禁止手動修改
