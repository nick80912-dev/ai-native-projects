...
> 同步時 Validator 會在 console 以 `[schema]` 前綴警告:缺少必要欄位/未知欄位/未知型別值/未知設定鍵,一律不崩潰。
...
| Type(別名:類型) | type | ✅ | 決定卡片型別,禁止程式猜測。值:購物/美食區/住宿/景點/渡船口/渡輪/租車點 |
...
| Currency | currency |  |
| Home Page | homepage | 預設分頁(目前固定 Today) |
