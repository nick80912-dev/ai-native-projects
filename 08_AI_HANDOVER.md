...
- 可重算 cold-navigation 證據固定 inline `d0405fe` 與 final asset `d88f5e5`：Today 中位數 171.70 → 158.00 ms（-7.98%）；每個樣本均記錄 HTML／App／asset／SW／timestamp identity，外部化通過 ≤10% kill gate。
