...
- Fresh final-fix pre-commit evidence: **86/86** top-level Node tests, full Playwright **175/175**, focused WebKit **21/21**, App／SW v108, document titles, 10 runtime assets, BUILTIN no-drift, manifest JSON, diff check, and offline Chromium `healthCheck: []`／`pageErrors: []` all pass. The final-fix implementer re-runs the committed-tree gates before handoff. Roll back with a forward version bump plus revert of v108 runtime registration, adapters, docs, tests, and version records together; never delete `sw.js` or change schema/data to roll back UI behavior.
