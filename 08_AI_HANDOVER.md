...
## v105 Today Hero dev-candidate handover

- v102 device/PWA acceptance is complete.
- v105 keeps the compact travel briefing and changes the active-trip Shopping projection to `地點 · 第一個優先分類 +N`; Hero visible and accessible output contains no product name. The exact next Shopping stop remains in the existing badge.
- The Hero summary preserves 44px touch/focus behavior and independently ellipsizes long location and category names. Pre-trip Today stays unchanged.
- Rollback boundary: forward-bump and revert the v105 Hero category model, renderer, CSS, tests, and version records together; do not alter schema, storage, Google Sheet data, or production/deployment records.
- Fresh gate evidence: 83/83 Node test files and 149/149 Playwright cases passed; document-title, app-version, runtime-assets, BUILTIN no-drift, manifest JSON, production-data health, and diff checks passed. Next action is device/PWA appearance and interaction acceptance on the dev candidate. Do not merge, deploy, or tag without approval.
...
- 這次 migration 不改 Ledger Schema 2.9／21 欄、Apps Script、個人備份 v9、SW lifecycle／cache strategy 或發布權限。v101／v102 裝置／PWA 外觀驗收均已由 Bar 完成；v102 的 HID facts 保留為歷史驗收紀錄。下一步由 Bar 在裝置／PWA驗收 v105 dev candidate 的 Today Hero 外觀與互動：採買地點後顯示第一優先分類與 +N、Hero 不顯示品名、精確下一站不重複、長站名與分類安全截斷，且無錯誤或水平 overflow。
