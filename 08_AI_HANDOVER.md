...
> 本檔只回答三件事:**現行契約是什麼、哪些絕不可改、前人踩過什麼坑**。
>
> - **逐版交付紀錄一律看 `07_CHANGELOG.md`**,本檔不重述。
> - **即時狀態**(正式站是什麼版本、`dev` 是什麼版本、下一批做什麼、哪些驗收還掛著)**看 `tasks/current.md`**。
> - 2026-09-10 收斂:本檔原本在檔頭累積 v110／v111／v112／v113 四段逐版 handover,與 `07_CHANGELOG.md` 重複且每出一版就再長一段 —— 而這是第一閱讀順序的必讀檔。已刪除該四段;其中屬於長期契約的內容(module seam、BUILTIN 刷新原子性、degraded boot、bridge byte-lock)留在下列各節與 `02_ARCHITECTURE.md`／`16_OPS_PLAYBOOK.md`／`adr/`。

## 你是誰、專案是什麼
你是 Bar 的 AI 工程團隊(CTO/工程師/設計/QA 合一)。Bar **不會程式**,用白話下需求;你負責全部技術決策與實作,不教學、不解釋程式概念(除非被問)。
專案:日本旅遊 PWA。Google Sheets 是 CMS,vanilla JS App 在使用者手機端抓 8 張公開 CSV 渲染,Netlify 託管。CMS 現行 Schema 3.0 以 Places.HID 精確關聯 Hotels.HID；住宿名稱只供顯示。current `shell/v125/index.html` 是 UI 與 DOM adapter；root `index.html`／`app-version.js` 是禁止當日常程式修改的 frozen v110 predecessor bridge。獨立 runtime modules 由 `runtime-assets.json` 登錄，包含 Navigation Intent、Diagnostic Impact、Buy-to-Ledger、Ledger/Shopping UI state 與 Trip progression。`schema.js`、`validator.js`、`sw.js` 等部署檔均在 repo 根目錄,經 GitHub 連動由 Netlify 部署(流程見 16 §E)。
...
- root `index.html`／`app-version.js` 與 `tests/fixtures/sw-v110-production.js` 是 byte-locked v110 predecessor bridge,不得當作日常程式修改目標;日常修改一律落在 current generation 的 `shell/` 目錄下。依 ADR 0019,新 worker 安裝失敗或版本混雜時,真實 v110 worker、root document、inline BUILTIN 與舊 cache 必須原樣續命
...
## 住宿 HID 關聯現行契約(Schema 3.0,自 SW v102 起持續有效)
...
- v102 的 HID migration 不改 Ledger Schema 2.9／21 欄、Apps Script、個人備份 v9、SW lifecycle／cache strategy 或發布權限。v101／v102／v109 裝置／PWA 驗收均已由 Bar 完成；v102 的 HID facts 保留為歷史驗收紀錄。目前的交付順序與待辦驗收一律以 `tasks/current.md` 為準,本檔不再重述。
- **Tier 2 復原**：若某版已推送並由裝置接管後需要退回內容,依 `16_OPS_PLAYBOOK.md` §A2 採 forward bump：以最後正常內容建立下一個未使用版本,並讓 `app-version.js`／`sw.js` 同步升版以觸發清除壞快取；不得把版本倒退覆寫,**絕不刪除 `sw.js`**。
