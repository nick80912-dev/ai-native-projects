...
Bar 要求「分帳頁面的優化與個人、團體分帳邏輯確認」。邏輯部分**不靠讀程式推論**,而是在真實 App 內直接呼叫計算函式跑不變量測試(純函式,不寫入任何資料);介面部分以既有成員身分(僅寫本機 `trip_member`,不送出)走查,表單一律不存檔,結束後清掉測試身分。

### 驗證通過(核心正確)

| 項目 | 方法 | 結果 |
|---|---|---|
| 均分總和守恆 | 隨機 20,000 例 | 0 失敗 |
| 團體淨額零和(JPY／TWD 各自) | 隨機 400 趟旅程 | 0 失敗,付款總額＝應付總額 |
| 照轉帳建議付完全部歸零 | 800 次 | 全部歸零,轉帳數 ≤ 人數 − 1 |
| 多品項雙幣守恆 | 2,996 例 | 0 失敗 |
| 正式／TEST 帳隔離、付款人不在分攤名單 | 定點 | 正確 |
| 真實資料 | — | PN 應付 Jane ¥1,500,其餘已結清,與畫面一致 |

### 修正

1. **編輯／更正「税抜」的多品項帳會重複加稅(現存 bug,稽核中新發現)**。實測 `税抜 10% [1000,500]`:新增 ¥1,650 → 編輯不改直接存 ¥1,815 → 再存 ¥1,997;有折扣時 ¥1,550 → ¥1,715 → ¥1,897。團體帳的**更正流程**同樣中招(¥1,650 → ¥1,815;單筆收據 ¥1,100 → ¥1,210)。成因:`ledgerDraftFromRecords()` 載入的品項金額是**已含稅**的最終金額,卻沿用原紀錄的 `priceMode:'excluded'`,存檔時再乘一次稅率。
   - 改法:新增 `ledgerAdoptFinalAmounts()`,多品項編輯與所有更正一律以 `税込` 載入,金額原封不動;原為 税抜 時記在 `sourcePriceMode`,稅區顯示「原紀錄為税抜;品項金額已含稅,改以税込編輯,避免重複加稅」。
   - **單品項刻意不走這裡**:單品的稅本來就只記錄、不重算金額,保留原 `priceMode` 才不會無故改動紀錄。
   - 真實團體資料的支出全部是 `included`,**沒有已被放大的帳**;個人帳在各人手機上,無法遠端檢查。
   - 已存在的紀錄一筆都不重算。
2. **結算幣別鎖定**。`Ledger Default Currency` 同時是全團結算幣別(ADR 0007),設定頁卻標為「預設輸入幣別 —— 新增記帳時預先選擇的幣別」。以真實資料實測:兩筆還款都以 JPY 確認,若有人把它改成 TWD,**已還清 ¥1,500 的哈囉祺會被要求再付 NT$315**,黃柏由已結清變回應收 NT$210。
   - 設定頁改標「結算幣別」並說明全團共用;區塊標題改為「匯率與結算幣別」。
   - 只要有任何還款紀錄(正式或 TEST 的 `settlement_claim`／`settlement_confirm`),另一個幣別按鈕即 `disabled`(`opacity:.45`,沿用 selection toolbar 慣例,否則與「未選取」外觀相同)並顯示原因;`saveLedgerSettings()` 在**送出網路請求前**另有同條件守門。同幣別只改匯率照常可存。
   - 直接改 Google Sheet 不在守門範圍內,已記入 ADR 0007 修訂。
3. **單品項稅區標明「僅記錄」**。單品項的「税込／税抜」「稅率」原本可選卻不影響金額,也沒有說明。稅區最上方補一行:「單品項的金額就是實付總額;價格方式與稅率只做記錄,不會加稅。要依未稅價自動加稅,請改用多品項。」
4. **分帳頁記住分頁,有未結清時先開團體**。原本每次開 App 都回到「個人」,PN 有 ¥1,500 待付也要多點一下才看得到。新增本機 key `trip_ledger_track` 記住上次選擇;本次開 App 第一次進分帳頁時,若自己在結算幣別仍有淨額(應付或應收)就先開團體。其他流程(採買連結直接開某筆紀錄)已明確指定分頁時不覆寫。

### 稽核時一度要改、查證後保留的兩項(前提修正)

稽核報告原把下列兩項列為待修,Bar 也一度選了要改;動工時發現**兩者都有既有測試明文守著,是刻意設計**,據實回報後 Bar 裁定維持原設計:

- **單品項的稅只記錄、不重算金額** —— 2026-07-19 Ledger 2.2 刻意定下(`ledger-multi-item.test.js`「single-entry tax metadata never recalculates the paid amount」),模型是「單品＝收據實付總額」。原本要改成「單品也套稅」,改為只補說明(上方第 3 點)。
- **個人帳代購不依身分過濾** —— 2026-07-18 起的設計(`ledger-proxy.test.js`「all historical proxy records remain visible regardless of current identity」)。個人帳跟著手機走,選錯身分期間記的代購是別人欠你的錢,不該因身分不同而消失。已一度實作過濾,跑 Node 測試時被這條斷言擋下,**撤回並在程式碼原處加註解說明這個不對稱是刻意的**。

**教訓(同 #31／#37／#46／#49)**:稽核用「規格寫 A、實作做 B」判斷缺陷時,要先查有沒有測試明文守著 B。這次兩項都是被 Node 測試當場擋下才發現,報告階段就該查。

### 稽核結論:不修的兩項

- **除不盡的餘數永遠給分攤名單第一位**。4 人、60 筆全員均分,第一位平均多付約 ¥22／NT$22,最後一位少付同額。**不修**:改分配法會讓所有已結清的歷史餘額重算。
- **團體 hero「與我相關旅程累計」顯示整張帳單總額**,不是個人負擔 —— v88 規格明寫「不宣稱是個人負擔」,屬產品決定,維持。

### 殘留(另開 backlog #50)

修正後,編輯／更正**總額永遠不變、輸入幣別的每個品項也不變**(1,500 例隨機);但 **税抜 帳的換算幣別**在品項之間最多會挪動 ±5(總額不變,税込 帳完全不受影響)。成因:換算幣別依原始未稅權重分配,權重沒有存下來,重載時只能以含稅金額代替。依 ADR 0007 另一幣純參考,只有「以非結算幣別輸入、且品項分攤成員不同」的帳會影響結算。

### 測試

- 新增 `tests/browser/ledger-money-integrity.spec.js` 五條:編輯／更正不改總額(1,500 例隨機,含輸入幣別逐品項)、税抜 帳以税込載入並提示、單品稅區說明、結算幣別鎖定且送出前擋下、有未結清先開團體、無欠款時記住分頁。**五個回退案例逐一拿掉修正實測皆會紅**(撤回代購過濾後,對應案例一併移除)。
- `tests/ledger-entry-settings.test.js`:設定頁文案斷言改為「結算幣別」,並斷言舊標籤「預設輸入幣別」已不存在、渲染前會檢查鎖定。
- 五筆發布說明視窗滾到 `v135` + `['v134','v133','v132','v131']`,v130 那筆滾出。
- 四個 gate、**95/95 Node**、**196/196 Playwright** 通過(191 + 本版新增 5)。
...
- Netlify deploy **`6ab48b3122589d00086439ed`**、`ready`、`commit_ref` = `9c4239f` 相符、`published_at` `2026-09-24T02:30:18Z`、`manual_deploy: false`,20 個新檔(含 `shell/v132/index.html`)、6 條 header rule 全過。`main` 上 merge commit 的 `sanity` 與 `browser-qa` 皆 success。
- **§F5 線上核對五項全過**:`sw.js` v132;`shell/v132/` 三件組皆 v132;root bridge 維持 **v110 未被誤升**;三處 `Cache-Control` 正確;**v111–v131 二十一個舊世代皆回 200**(ADR 0019)。另線上實查日期標頭、`opacity:.9` 規則、v132 發布說明皆已上線,重複的「,查看完整行程」為 0 次,v127 那筆已滾出五筆視窗。
- **G6 完成**:annotated tag `production-v132` 指向 `9c4239f`。tag 在真機回報之後才建立,訊息直接寫入驗證結果。
- **G1 經 Bar 裁定跳過**(同 v114–v131)。
- **真機驗證通過(2026-09-24)**:Bar 在手機上回報驗證 OK。
  - **一項未另行確認**:發布後曾請 Bar 特別在 mist／tea 主題下看新標頭 —— 量測為 5.59／5.62:1,但這兩個正是沿用 `.75` 時不及格的主題。回報未逐項指明是否切換主題看過。若日後有人反映出發前的日期標頭看不清楚,**先確認當時使用的主題**。
  - **BB4 八項維持未驗**。
- **backlog 狀態**:#48 已結案。剩餘 **#3／#12／#20／#25／#28／#29／#38／#41／#42／#46／#49**,外加 **E1**、**E2** 兩項大型重構;#27 底下另記一個已知缺口。
...
- Netlify deploy **`6ab3807ea40c3f00081f8023`**、`ready`、`commit_ref` = `b4182e9` 相符、`published_at` `2026-09-23T07:32:29Z`、`manual_deploy: false`,31 個新檔(含 v127–v130 四個 generation 目錄)。
- **§F5 線上核對五項全過**:`sw.js` v130;`shell/v130/` 三件組皆 v130;root bridge 維持 **v110 未被誤升**;三處 `Cache-Control` 正確;**v111–v129 十九個舊世代皆回 200**(ADR 0019)。另線上實查本批四個版本的改動皆已上線 ——「已處理」3 次、「這一站」5 次、「還有」5 次、`button:focus-visible` 8 次、`outline-color:#fff` 1 次。
- **一個值得記下的輪詢陷阱**:部署後前 12 次輪詢都讀到 v126,一度看起來像 CDN 沒更新。實際原因是那些請求全部打在 `published_at`(07:32:29)**之前** —— **不是快取問題**。日後遇到同樣情形先比對 `published_at`,不要據此去清 CDN 或重新部署。
- **G6 完成**:annotated tag `production-v130` 指向 `b4182e9`。**v127／v128／v129 未單獨建 tag**(與 v130 同批發布,同 v120 的處理)。
- **G1 經 Bar 裁定跳過**(同 v114–v126)。
- **真機驗證通過(2026-09-23)**:Bar 在手機上回報驗證 OK。**本次順序與 v125／v126 相反 —— tag 在真機回報之後才建立**,因此 `production-v130` 的 tag 訊息直接寫入驗證結果,不需再像前兩次那樣靠本文件補正。
  - **一項未另行確認**:v130 的鍵盤焦點環需**外接鍵盤**按 Tab 才驗得到,Bar 的回報未逐項指明是否走過。若日後發現焦點環在某處失效,**先確認該項當初是否真的驗過**。
  - **BB4 八項維持未驗**(同 2026-09-15 裁定):整體試用無異常不等同逐項走過。
- **backlog 狀態**:#40／#43／#44／#45 皆已結案。剩餘 **#3／#12／#20／#25／#28／#29／#38／#41／#42／#46／#47／#48／#49**,外加 **E1**(角色 token 採用率 5/103)與 **E2**(寫死 hex)兩項大型重構。
- **本文件的一處既有錯位(未一併更動)**:檔案最上方的「2026-09-13 — v124 正式發布」區塊位置不對 —— 其餘版本的發布紀錄一律緊接在該版 candidate 區塊之上(v123／v125／v126 皆如此),只有 v124 那則被放到全檔最前面。內容正確,僅位置有誤;**移動區塊會動到不屬於本批的紀錄,留待專門的文件整理處理**。
