...
- **撤銷限制**：新增純函式 `settlementConfirmRevokeEligibility(entry,allEntries,actor,now)` 回傳 `{allowed,reason,expiresAt}`，須同時為原收款者、有效 `settlement_confirm`、該 key 最新 generation，且距 `response.time` ≤ 24 小時（23:59:59.999 與正好 24:00:00.000 可撤銷、+1ms 不可）。UI 不符資格即不輸出按鈕，handler 直接呼叫仍被阻擋；時間比較使用 ISO `record.time`，不依顯示時區字串。
